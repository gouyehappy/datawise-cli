...
| G13 | 组织级数据发现 | partial | 命令面板跨库搜表 + GET /api/discovery/search；**数据目录 Tab** + 血缘跳转 + **分面筛选**（kind / connection / owner）；见 [DISCOVERY.md](./DISCOVERY.md)。缺无查询浏览 / 指标血缘 / 标签分面 | 语义层有了，发现体验仍弱 |
