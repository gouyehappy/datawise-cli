...
| G13 | 组织级数据发现 | partial | 命令面板跨库搜表 + GET /api/discovery/search；**数据目录 Tab** + 表/视图/指标血缘跳转（指标经 relatedTables，多表可选）+ **分面筛选** + **无查询浏览**；见 [DISCOVERY.md](./DISCOVERY.md)。缺标签分面 / 分页浏览 | 语义层有了，发现体验仍弱 |
