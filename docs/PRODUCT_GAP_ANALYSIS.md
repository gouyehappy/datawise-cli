...
| G13 | 组织级数据发现 | partial | 命令面板跨库搜表 + GET /api/discovery/search；**数据目录 Tab** + 表/视图/指标血缘跳转（指标经 relatedTables）+ **分面筛选** + **无查询浏览**；见 [DISCOVERY.md](./DISCOVERY.md)。缺标签分面 / 分页浏览 / 多关联表选择 | 语义层有了，发现体验仍弱 |
...
| G15 | 可调度数据质量规则 | partial | 定时任务 data_quality + Explorer **数据质量**目录 + locking + gate API + **内置规则模板** + **多环境对照门禁**（referenceConnectionId + scopes）；见 [DATA_QUALITY.md](./DATA_QUALITY.md)。缺用户自定义模板库 / 规则跨环境自动配对 | 质量治理难产品化 |
...
| S2 | **联邦 JOIN 规模边界** | partial | 内存 INNER JOIN + 硬上限 + hasMore；Grace hash 落盘；外层 WHERE 下推 + 残差 **OR / IN / NOT IN / 裸 NOT**；见 [FEDERATED_JOIN_BOUNDS.md](./FEDERATED_JOIN_BOUNDS.md)。仍缺 OR 下推进源 SQL / 函数 | 限流 / 溢出策略 / 文档化边界；可选下推或分批 |
