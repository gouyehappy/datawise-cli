...
| S2 | **联邦 JOIN 规模边界** | partial | 内存 INNER JOIN + 硬上限（默认 1k / 硬顶 10k）+ 笛卡尔积拒绝 + `hasMore`；**Grace hash 落盘**（构建侧 >512）；**外层 WHERE 谓词下推**（单别名进源 SQL，跨别名内存过滤）；见 [FEDERATED_JOIN_BOUNDS.md](./FEDERATED_JOIN_BOUNDS.md)。仍缺复杂表达式/OR 下推 | 限流 / 溢出策略 / 文档化边界；可选下推或分批 |
