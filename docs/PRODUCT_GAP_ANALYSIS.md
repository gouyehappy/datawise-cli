...
| S2 | **联邦 JOIN 规模边界** | partial | 内存 INNER JOIN + 硬上限 + hasMore；Grace hash 落盘；外层 WHERE 下推 + 残差谓词/函数目录已闭环（OR/IN/IS NULL/LIKE[+ESCAPE]/BETWEEN/TRIM|LENGTH|ABS|COALESCE|NULLIF|CONCAT|\|\||SUBSTR/…）；见 [FEDERATED_JOIN_BOUNDS.md](./FEDERATED_JOIN_BOUNDS.md)。缺进一步限流 UX / 分批执行 | 限流 / 溢出策略 / 文档化边界；可选下推或分批 |
