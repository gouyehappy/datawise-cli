...
describe('kafka-feature-tree.service', () => {
    it('builds virtual feature nodes under kafka connection', () => {
        ...
        assert.equal(children[2].meta, 'table-publish')
    })
    ...
    it('injects children once for kafka connection', () => {
        ...
        assert.equal(connection.children?.length, 3)
    ...
    it('adds missing feature nodes to existing kafka children', () => {
        const connection = {
            id: 'kafka-1',
            label: 'local',
            type: 'connection' as const,
            dbType: 'kafka' as const,
            children: [
                {
                    id: 'kafka-1:kafka:feature:topics',
                    label: 'topics',
                    type: 'kafka-feature' as const,
                    dbType: 'kafka' as const,
                    meta: 'topics',
                },
            ],
        ...
        ensureKafkaConnectionFeatureChildren(connection)
        assert.equal(connection.children?.length, 3)
        assert.ok(connection.children?.some((child) => child.meta === 'table-publish'))
    })
    ...
    it('parses feature id from node meta', () => {
        ...
        assert.equal(parseKafkaFeatureId({type: 'kafka-feature', meta: 'table-publish'}), 'table-publish')
