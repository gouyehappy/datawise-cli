...
import {ensureSshConnectionFeatureChildren} from '@/features/explorer/services/ssh-feature-tree.service'
import {ensureYarnConnectionFeatureChildren} from '@/features/explorer/services/yarn-feature-tree.service'

/** Kafka / Redis / YARN / SSH 连接展开时注入虚拟功能子节点 */
export function ensureFlatConnectionFeatureChildren(node: TreeNode): void {
    ...
    if (node.dbType === 'ssh') ensureSshConnectionFeatureChildren(node)
}

export function isConnectionFeatureNode(node: Pick<TreeNode, 'type'>): boolean {
    return node.type === 'kafka-feature'
        || node.type === 'redis-feature'
        || node.type === 'yarn-feature'
        || node.type === 'ssh-terminal'
        || node.type === 'ssh-script-records'
        || node.type === 'ssh-script-record'
}
