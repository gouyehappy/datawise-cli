...
import static org.apache.datawise.sqlflow.analyzer.SemanticExceptions.ambiguousAttributeException;
...
public class Scope
{
     ...
     */
    public Optional<AsteriskedIdentifierChainBasis> resolveAsteriskedIdentifierChainBasis(
            QualifiedName identifierChain,
            AllColumns selectItem)
    {
        ...
        if (length >= 2) {
            scopeForFieldReference = findLocally(scope -> scope.getRelationType()
                    .getAllFields().stream()
                    .anyMatch(field ->
                            field.matchesPrefix(Optional.of(QualifiedName.of(identifierChain.getParts().get(0))))
        ...
        if (scopeForTableReference.isPresent() && scopeForFieldReference.isPresent()) {
            throw semanticException(selectItem, "Reference '%s' is ambiguous", identifierChain);
        }

        if (scopeForTableReference.isPresent()) {
            return Optional.of(new AsteriskedIdentifierChainBasis(
                    BasisType.TABLE,
                    scopeForTableReference,
                    Optional.of(scopeForTableReference.get().getRelationType())));
        }

        if (scopeForFieldReference.isPresent()) {
            return Optional.of(new AsteriskedIdentifierChainBasis(BasisType.FIELD, Optional.empty(), Optional.empty()));
        }
    ...
    {
        QualifiedName qualifiedName = ExpressionTreeUtils.asQualifiedName(expression);
    ...
    public static class AsteriskedIdentifierChainBasis
    {
                ...
                Optional<RelationType> relationType)
        {
            ...
            this.relationType = requireNonNull(relationType, "relationType is null");
            checkArgument(basisType == BasisType.FIELD || scope.isPresent(), "missing scope");
            checkArgument(basisType == BasisType.FIELD || relationType.isPresent(), "missing relationType");
        }
