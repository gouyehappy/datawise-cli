...
import org.apache.datawise.sqlflow.parser.antlr4.SqlFlowParserBaseVisitor;
...
import static org.apache.datawise.sqlflow.parser.antlr4.SqlFlowLexer.SYSTEM_TIME;
import static org.apache.datawise.sqlflow.parser.antlr4.SqlFlowLexer.TIME;
import static org.apache.datawise.sqlflow.parser.antlr4.SqlFlowLexer.TIMESTAMP;
import static org.apache.datawise.sqlflow.parser.antlr4.SqlFlowLexer.VERSION;
import static org.apache.datawise.sqlflow.tree.window.SkipTo.skipPastLastRow;
import static org.apache.datawise.sqlflow.tree.window.SkipTo.skipToFirst;
import static org.apache.datawise.sqlflow.tree.window.SkipTo.skipToLast;
import static org.apache.datawise.sqlflow.tree.window.SkipTo.skipToNextRow;
import static org.apache.datawise.sqlflow.tree.window.rowPattern.AnchorPattern.Type.PARTITION_END;
import static org.apache.datawise.sqlflow.tree.window.rowPattern.AnchorPattern.Type.PARTITION_START;
import static org.apache.datawise.sqlflow.tree.window.rowPattern.PatternSearchMode.Mode.INITIAL;
import static org.apache.datawise.sqlflow.tree.window.rowPattern.PatternSearchMode.Mode.SEEK;
...
public class AstBuilder extends SqlFlowParserBaseVisitor<Node>
{
        ...
        if (context.FETCH() != null) {
            ...
            if (context.fetchFirst != null) {
                if (context.fetchFirst.INTEGER_VALUE() != null) {
                    rowCount = Optional.of(new LongLiteral(
                            getLocation(context.fetchFirst.INTEGER_VALUE()),
                            context.fetchFirst.getText()));
                } else {
                    rowCount = Optional.of(new Parameter(
                            getLocation(context.fetchFirst.QUESTION_MARK()),
                            parameterPosition));
                    parameterPosition++;
                }
            }
            limit = Optional.of(new FetchFirst(
                    Optional.of(getLocation(context.FETCH())),
                    rowCount,
                    context.TIES() != null));
            ...
            if (context.limit.ALL() != null) {
                ...
                rowCount = new LongLiteral(
                        getLocation(context.limit.rowCount().INTEGER_VALUE()),
                        context.limit.getText());
            } else {
    ...
    public Node visitMultipleGroupingSets(SqlFlowParser.MultipleGroupingSetsContext context)
    {
        return new GroupingSets(
                getLocation(context), context.groupingSet().stream()
    ...
    {
        List<ParserRuleContext> terms = flatten(
                context, element -> {
    ...
    public Node visitTableName(SqlFlowParser.TableNameContext context)
    {
        if (context.queryPeriod() != null) {
            return new Table(
                    getLocation(context),
                    getQualifiedName(context.qualifiedName()),
                    (QueryPeriod) visit(context.queryPeriod()));
        }
        return new Table(getLocation(context), getQualifiedName(context.qualifiedName()));
    }
    ...
    public Node visitListagg(SqlFlowParser.ListaggContext context)
    {
        ...
        StringLiteral separator = context.string() == null ? new StringLiteral(
                getLocation(context),
    ...
    public Node visitNormalize(SqlFlowParser.NormalizeContext context)
    {
        ...
        return new FunctionCall(
                getLocation(context),
                QualifiedName.of(ImmutableList.of(new Identifier(
                        "normalize",
                        true))), // delimited to avoid ambiguity with NORMALIZE SQL construct
    ...
    public Node visitFunctionCall(SqlFlowParser.FunctionCallContext context)
    {
        ...
        if (name.toString().equalsIgnoreCase("if")) {
            check(
                    context.expression().size() == 2 || context.expression().size() == 3,
        ...
        if (name.toString().equalsIgnoreCase("$internal$bind")) {
            check(
                    ...
                    "The '$internal$bind' function must have at least one arguments",
                    context);
            ...
            check(
                    processingMode == null,
                    "Running or final semantics not valid for '$internal$bind' function",
                    context);
        ...
        if (context.label != null) {
            arguments = ImmutableList.of(new DereferenceExpression(
                    getLocation(context.label),
                    (Identifier) visit(context.label)));
        }
    ...
    public Node visitQuantifiedPrimary(SqlFlowParser.QuantifiedPrimaryContext context)
    {
        ...
        if (context.patternQuantifier() != null) {
            return new QuantifiedPattern(
                    getLocation(context),
                    primary,
                    (PatternQuantifier) visit(context.patternQuantifier()));
        }
        return primary;
    }
    ...
    public Node visitQualifiedArgument(SqlFlowParser.QualifiedArgumentContext context)
    {
        return new PathElement(
                getLocation(context),
                (Identifier) visit(context.identifier(0)),
                (Identifier) visit(context.identifier(1)));
    }
                ...
                case UNICODE_SEQUENCE:
                    ...
                    if (charactersNeeded == escapedCharacterBuilder.length()) {
                        ...
                        if (Character.isSupplementaryCodePoint(codePoint)) {
                            ...
                            check(
                                    !Character.isSurrogate(currentCodePoint),
                                    format(
