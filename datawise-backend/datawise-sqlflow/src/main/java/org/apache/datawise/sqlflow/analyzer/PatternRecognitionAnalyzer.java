...
public class PatternRecognitionAnalyzer
{
            ...
            Optional<SkipTo> skipTo)
    {
        ...
        Set<String> primaryLabels = ExpressionTreeUtils
                .extractExpressions(ImmutableList.of(pattern), Identifier.class)
                .stream()
        ...
        for (VariableDefinition definition : variableDefinitions) {
            ...
            ExpressionTreeUtils.extractExpressions(ImmutableList.of(expression), FunctionCall.class).stream()
                    .filter(functionCall -> functionCall
                            .getProcessingMode()
                            .map(mode -> mode.getMode() == ProcessingMode.Mode.FINAL)
                            .orElse(false))
                    .findFirst()
                    .ifPresent(functionCall -> {
                        throw SemanticExceptions.semanticException(
                                functionCall.getProcessingMode().get(),
        ...
        preOrder(pattern)
                ...
                .forEach(quantifier -> {
                    ...
                    ranges.put(
                            ...
                            new Analysis.Range(atLeast.map(Math::toIntExact), atMost.map(Math::toIntExact)));
                });
    ...
    public static void validateNoPatternSearchMode(Optional<PatternSearchMode> patternSearchMode)
    {
        patternSearchMode.ifPresent(mode -> {
            throw semanticException(
                    mode,
                    "Pattern search modifier: %s is not allowed in MATCH_RECOGNIZE clause",
                    mode.getMode());
        });
    }

    public static void validatePatternExclusions(
            Optional<PatternRecognitionRelation.RowsPerMatch> rowsPerMatch,
            RowPattern pattern)
    {
    ...
    public static void validateNoPatternAnchors(RowPattern pattern)
    {
        preOrder(pattern)
                ...
                .ifPresent(anchor -> {
                    throw SemanticExceptions.semanticException(
                            anchor,
                            "Anchor pattern syntax is not allowed in window");
                });
    }

    public static void validateNoMatchNumber(
            List<MeasureDefinition> measures,
            List<VariableDefinition> variableDefinitions,
            Set<NodeRef<FunctionCall>> patternRecognitionFunctions)
    {
        List<Expression> expressions = Streams.concat(
                        measures.stream()
                                .map(MeasureDefinition::getExpression),
        ...
        expressions.forEach(expression -> preOrder(expression)
                ...
                .ifPresent(matchNumber -> {
                    throw SemanticExceptions.semanticException(
                            matchNumber,
                            "MATCH_NUMBER function is not supported in window");
                }));
    }
